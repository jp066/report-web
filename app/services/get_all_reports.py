...
def report_list():
   ...
   "User-Agent": requests.utils.default_user_agent()
    ...
    response = requests.post(URL, auth=AUTH, data=xml_text, headers=headers, verify=settings.SOAP_VERIFY_SSL)
    
    print(f"Status Code: {response.status_code}")
    print(f"Response Text (primeiros 500 chars): {response.text[:500]}")
    
    if response.status_code not in [200, 202]:
        raise RuntimeError(f"Erro ao obter a lista de relatórios: Status code {response.status_code}")
...
def formatted_report_list():
    ...
    formatted = [{"id_report": r["id_interno"], "report_name": r["nome_relatorio"]} for r in reports]
    return formatted
